...
Outline:
1. Introduction, definition of D-symbols
2. We could enumerate every edge (or face) transitive D-symbols (these are
special)
3. Definition of a splitting in D-symbols
4. Connection with Thurston's geometrization conjecture (E2 splitting=cut along
tori)
5. Algorithm for finding splittings
6. Illustrations (output of the program)
...
\begin{frame}
  ...
  \begin{itemize}
	...
	\item The values of the first diagonal above and below the main has to
	  be $\ge2$\mode<article>{. In case of nice tilings in 3 dimensions
...
%%%%%%%%%%% Eddig muszaj elmondani
%Emlites szinten kitero az eltranzitiv D-szimbolumokra
...
\begin{frame}
  ...
  3-manifolds in the conjecture are trivially orbifolds
...
\begin{frame}[plain]
  ...
  \only<2>{That looks like a screenshot, let's see the
  original.\mode<article>{Constructing a visualisation of the fundamental domain
  for edge transitive tilings is pretty straightforward: Take the 1 or 2 simplex
  edges between 0 and 1 vertices, and put around them the simpleces using their
  single parameter.}}
  \begin{figure}
